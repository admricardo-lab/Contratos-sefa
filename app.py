...
# Railway volume ou diretório do arquivo
...
def init_db():
    ...
    if count == 0:
        ...
        if os.path.exists(seed_file):
            ...
            db.commit()
            print(f'[init_db] Seeded {len(records)} records.')
    else:
        print(f'[init_db] DB ok — {count} records.')
    db.close()
...
def recalc_dias(dt_fim):
    if not dt_fim:
        return None
    try:
        fim = datetime.strptime(str(dt_fim)[:10], '%Y-%m-%d').date()
...
def list_contratos():
    ...
    args = []
    if q:
        sql += ''' AND (contrato LIKE ? OR fornecedor LIKE ? OR cnpj LIKE ?
                     OR descricao_servico LIKE ? OR conta_contabil LIKE ?
                     OR nro_aditivo LIKE ? OR ug LIKE ?)'''
        args += [f'%{q}%'] * 7
    if sit:
        sql += ' AND situacao = ?'
        args.append(sit)
...
def get_contrato(cid):
    row = get_db().execute('SELECT * FROM contratos WHERE id=?', (cid,)).fetchone()
    if not row:
...
def update_contrato(cid):
    ...
    ''', (
        ...
        data.get('valor_vigente',0), data.get('valor_mensal',0),
        data.get('conta_contabil',''), data.get('descricao_servico',''),
        recalc_dias(data.get('dt_fim')), data.get('situacao','NORMAL'), cid
    ))
    db.commit()
    row = get_db().execute('SELECT * FROM contratos WHERE id=?', (cid,)).fetchone()
    if not row:
...
# Roda init_db() na importação — funciona com gunicorn e python direto
init_db()
